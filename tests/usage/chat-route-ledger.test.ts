...
test('chat route records trusted usage and returns the traced response contract', () => {
  const source = fs.readFileSync(
    path.join(process.cwd(), 'app/api/ai/chat/route.ts'),
    'utf8',
  );
  assert.match(source, /buildTextLedgerEntry/);
  assert.match(source, /recordUsageRequired/);
  assert.doesNotMatch(source, /recordUsageBestEffort/);
