...
async function providerJson(response: Response, context: ProviderJsonContext) {
  ...
  const detailedResponse = context.operation === 'submit' || !response.ok;
  logServerEvent('wetoken_video_exchange', {
    ...
    auditDetail: detailedResponse ? 'full_context' : 'summary',
    ...(detailedResponse ? {
      responseHeaders: safeProviderHeaders(response.headers),
...
async function providerFetch(
  ...
  logServerEvent('wetoken_video_exchange', {
    ...
    exchangeId,
    stage: 'request_sent',
    auditDetail: operation === 'submit' ? 'full_context' : 'summary',
