...
实际 Docker 主机上的 `scripts/service-monitor.sh` 每轮把 Docker、NAS、App、PostgreSQL、Tunnel、磁盘和 App 错误检查写入 `observability_service_events` / `observability_error_events`。Tunnel 检查不能只依赖 `cloudflared tunnel ready`：只要配置了 `PROVIDER_MEDIA_URL`，监控还会从宿主机探测该公网媒体地址，将 HTTP 4xx 视为“隧道已到达应用”（例如无参数探测得到 404），将 5xx、超时和连接失败视为不可用；连续失败达到阈值后才重启 connector，并使用冷却窗口避免网络中断时反复重启。App 错误采集只匹配顶层 `level=error/critical`、失败/未知 `outcome`、失败 `stage` 或明确的 fatal/panic/exception 文本，不会把成功日志中的嵌套 `error: null` 当成故障；`app-errors` 同一健康状态不重复写事件，只记录状态变化。既有健康心跳与 App 错误 telemetry 在独立后台进程中 best-effort 上报；持久 outbox 则在本轮健康检查后同步尝试投递，每轮最多 5 条、每条最多等待 2 秒。网络或 App 不可用时事件继续留在本机队列。`scripts/report-scheduler.sh` 每 5 分钟调用一次内部 `/api/observability/report-runner`；每轮最多生成 4 份报表，scheduler 失败或历史补算未完成时下一轮继续重试。内部接口使用 `x-fg-observability-secret`，优先读取 `FG_OBSERVABILITY_SECRET`，兼容回退到 `SESSION_SECRET`。

HTTPS Nginx gateway 将 5xx 请求以 JSON 写入容器 stdout，字段包含不带 query 的 URI、HTTP/upstream 状态、耗时以及 trace/request ID；不记录请求参数、headers、cookie 或 body。`service-monitor.sh` 按持久游标读取这些记录，先写入宿主机 `~/Library/Application Support/fg-studio-monitor/outbox`，再通过受 secret 保护的 `/api/observability/error-events` 送入 `source=infra`。App 暂时不可用时事件留在 outbox，成功收到 2xx 后才删除；固定 `eventKey` 使重叠扫描和重试可幂等入库。NAS supervisor 的故障、恢复和状态过渡也会进入同一 outbox，再写入 `observability_service_events`，并保留原始状态与发生时间。这样 `/admin/logs` 能显示 Nginx 5xx 和 supervisor 状态时间线；此前已轮转或未采集的历史 Docker 日志不会自动回填。
