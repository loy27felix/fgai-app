import { createPluginStorage, emitCanvasEvent, onCanvasEvent } from "@/reference/infinite-canvas/src/lib/canvas/canvas-event-bus";
import { getNodePluginId } from "@/reference/infinite-canvas/src/lib/canvas/node-registry";
...
export function buildNodeContext(host: CanvasPluginHost, node: CanvasNodeData, theme: CanvasTheme, scale: number, isSelected = false): CanvasNodeContext {
    ...
    return {
        ...
        ai: host.getAiForNode ? host.getAiForNode(node.id) : host.ai,
