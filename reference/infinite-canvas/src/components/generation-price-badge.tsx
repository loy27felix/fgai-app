...
import { estimateAudioPrice, estimateImagePrice, estimateVideoPrice, normalizeVideoPriceResolution } from "@/lib/usage/pricing";
...
export function GenerationPriceBadge({ kind, model, size = "", imageQuality = "", count = 1, duration = 0, resolution = "", ratio = "", hasVideoReference = false, imageReferenceCount = 0, videoReferenceSeconds = 0, audioDurationSeconds = 0, className = "" }: GenerationPriceBadgeProps) {
    ...
    const audioSeconds = Math.max(0, Number(audioDurationSeconds) || 0);
    const pricing = kind === "image"
        ? estimateImagePrice(modelName, imagePriceSize, { referenceCount: imageReferenceCount })
        : kind === "audio"
            ? estimateAudioPrice({ model: modelName, durationSeconds: audioSeconds || undefined })
            : estimateVideoPrice({
            model: modelName,
    ...
    const unitLabel = kind === "image"
        ? (units > 1 ? String(units) + " 张" : "1 张")
        : kind === "audio"
            ? (audioSeconds > 0 ? `${audioSeconds} 秒` : "按实际时长")
            : String(videoSeconds) + " 秒";
    const audioPerMinuteCny = kind === "audio" && pricing ? Number(pricing.snapshot.raw_per_minute_cny || 0) : 0;
    ...
    return (
        <div className={`inline-flex min-w-0 items-center gap-2 rounded-full border px-3 py-2 text-xs ${className}`} style={{ borderColor: "rgba(120,113,108,.28)", color: "var(--foreground, #292524)", background: "color-mix(in srgb, var(--card, #fff) 78%, transparent)" }} title={pricing ? `按已确认价格预估：${pricing.snapshot.note || ""}` : "当前模型或参数不在价格目录中"}>
            ...
            ) : kind === "audio" && audioSeconds <= 0 ? (
                <span className="truncate">
                    预计 <strong className="font-semibold">¥{audioPerMinuteCny.toFixed(2)}/分钟</strong>
                    <span className="ml-1 opacity-60">（按实际生成时长结算）</span>
                </span>
            ) : (
                <span className="truncate">
