...
import { ChevronDown, ChevronRight, FileAudio, Folder, FolderOpen, Image as ImageIcon, Plus, Search, Upload, Video } from "lucide-react";
...
export const CanvasMaterialLibraryTab = memo(function CanvasMaterialLibraryTab({ onInsert, theme }: { onInsert: (payload: MaterialInsertPayload) => void; theme: CanvasTheme }) {
    ...
    return <div className="flex h-full min-h-0 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
            ...
            {visibleItems.length ? <div className="grid grid-cols-2 gap-2">{visibleItems.map((item) => <MaterialCard key={item.id} item={item} folders={folders} onInsert={() => onInsert(materialToInsertPayload(item))} onMove={(folderId) => updateItem(item.id, { folderId })} />)}</div> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="这里还没有素材" className="pt-10" />}
        </div>
    </div>;
});
...
function MaterialCard({ item, folders, onInsert, onMove }: { item: MaterialItem; folders: MaterialFolder[]; onInsert: () => void; onMove: (folderId: string | null) => void }) {
