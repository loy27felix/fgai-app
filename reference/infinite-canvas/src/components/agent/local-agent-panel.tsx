    ...
    <section className="fg-agent-panel" aria-label="FG Agent 对话">
      ...
      <div className="fg-agent-message-wrap">
        <div ref={scrollRef} className="fg-agent-messages" onScroll={handleMessagesScroll}>
          {loading ? <div className="fg-agent-empty">正在加载对话…</div> : messages.length ? messages.map((message) => <article className={message.role === "user" ? "fg-agent-message user" : "fg-agent-message assistant"} key={message.id}><div className="fg-agent-message-role">{message.role === "user" ? "你" : "Agent"}</div><div className="fg-agent-message-text">{textOf(message)}</div>{message.content?.usage ? <small className="fg-agent-usage">{message.content.usage.total_tokens ? `${message.content.usage.total_tokens.toLocaleString()} tokens` : ""}</small> : null}</article>) : <div className="fg-agent-empty"><Sparkles size={22} /><strong>先把想法变成可执行方案</strong><span>可写提示词、拆解镜头、分析参考图和规划节点连接。</span><small className="fg-agent-scope-note">这里的公司模型不会直接改动画布；请切换到已连接的「本机 Codex」执行真实节点操作。</small></div>}
