...
const FALLBACK_RATIOS = ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9"];
...
export function CompanyVideoSkillFlow({ skills, planningModelLabel, videoModels, storyboardModels, canvasReady, onPrepare, onStart, onClose }: Props) {
  ...
  const videoSpec = useMemo(() => getVideoModel(modelName(videoModel)), [videoModel]);
  const videoResolutions = videoSpec?.resolutions || ["480p", "720p", "1080p", "4K"];
  const videoRatios = (videoSpec?.ratios || FALLBACK_RATIOS).filter((item) => item !== "adaptive");
  const videoDurations = Array.from({ length: (videoSpec?.maxDuration || 15) - (videoSpec?.minDuration || 4) + 1 }, (_, index) => (videoSpec?.minDuration || 4) + index);
  ...
  return (
    <section className="fg-company-video-flow" aria-label="公司模型 Skill 视频制作">
      ...
      <div className="fg-company-video-flow-body">
        ...
        {step === 1 ? <div className="fg-company-video-flow-question fg-company-video-flow-specs"><span className="fg-company-video-flow-question-number">02 / 04</span><h3>定义镜头节奏与制作规格</h3><label>主体、故事和关键动作<textarea value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="例如：小女孩第一次触碰玩具吉他，发现不同区域会发出不同音色；产品结构必须准确。" /></label><label>画面与镜头方向<textarea value={visualDirection} onChange={(event) => setVisualDirection(event.target.value)} placeholder="例如：明亮午后、玩具广告质感、由近及远的跟拍，避免文字、水印和畸形手部。" /></label><div className="fg-company-video-flow-selects"><label>视频模型<select value={videoModel} onChange={(event) => { const nextModel = event.target.value; const nextSpec = getVideoModel(modelName(nextModel)); const nextResolutions = nextSpec?.resolutions || videoResolutions; const nextRatios = (nextSpec?.ratios || FALLBACK_RATIOS).filter((item) => item !== "adaptive"); const nextMinDuration = nextSpec?.minDuration || 4; const nextMaxDuration = nextSpec?.maxDuration || 15; setVideoModel(nextModel); if (!nextResolutions.includes(videoResolution)) setVideoResolution(nextResolutions[0]); if (!nextRatios.includes(ratio)) setRatio(nextRatios[0] || "16:9"); if (duration < nextMinDuration || duration > nextMaxDuration) setDuration(nextMinDuration); }}>{videoModels.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>清晰度<select value={videoResolution} onChange={(event) => setVideoResolution(event.target.value)}>{videoResolutions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>画幅<select value={ratio} onChange={(event) => setRatio(event.target.value)}>{videoRatios.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label>每段时长<select value={duration} onChange={(event) => setDuration(Number(event.target.value))}>{videoDurations.map((item) => <option key={item} value={item}>{item} 秒</option>)}</select></label><label>视频段数<select value={segmentCount} onChange={(event) => setSegmentCount(Number(event.target.value))}>{[1, 2, 3, 4].map((item) => <option key={item} value={item}>{item} 段</option>)}</select></label><label>分镜图模型<select value={storyboardModel} onChange={(event) => setStoryboardModel(event.target.value)}>{storyboardModels.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></div><p className="fg-company-video-flow-spec-note">每一段先生成 1 张分镜图，再把该图连到对应视频节点；分段越多，连续性控制越细，也会增加预计费用。</p></div> : null}
