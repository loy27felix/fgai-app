import { isSiteTool, SITE_TOOL_LABELS } from "@/reference/infinite-canvas/src/lib/agent/agent-site-tools";
import { summarizeCanvasAgentOps, type CanvasAgentOp } from "@/reference/infinite-canvas/src/lib/canvas/canvas-agent-ops";
import { randomId } from "@/reference/infinite-canvas/src/lib/utils";
import { useAgentStore, type AgentAttachment, type AgentChatItem, type AgentEventLog, type AgentTokenUsage } from "@/reference/infinite-canvas/src/stores/use-agent-store";
...
export function formatAgentEventLog(event: AgentEventPayload) {
    ...
    if (event.type === "item.completed" && isMcpToolItem(item)) { const errorMessage = item?.error?.message; return { title: item?.error ? "工具失败" : "工具完成", text: `${toolName(String(item?.tool || ""))}${errorMessage ? ` · ${errorMessage}` : ""}` }; }
