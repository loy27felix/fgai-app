...
type Topic = { id: number; title: string; original: string; plot: string; markets: string[] | string; tier: string; form?: string; style: string; confidence: string; blocked?: boolean; source_rights: string; conflict: string; source_name: string; selection_group?: string; selected_by?: string; selected_at?: string };
...
const topics = [...topicData as unknown as Topic[], ...teamSelectedTopics as Topic[]];
function topicSnapshot(topic: Topic) {
  return {
    ...
    selectedBy: topic.selected_by, selectedAt: topic.selected_at,
  };
}
...
function Desk({ demo = false, actor = demoActor, appearance = "light", onBackToWorkspace, onOpenWorkspace }: { demo?: boolean; actor?: Actor; appearance?: "light" | "dark"; onBackToWorkspace?: () => void; onOpenWorkspace?: (projectId: string) => void }) {
  ...
  function openCreate(topic?: Topic) {
    const source = topic?.selection_group
      ? `${topic.selection_group}已选题 · ${topic.selected_by} · ${topic.selected_at}；故事原型、市场与权利待补充`
  ...
  const filteredTopics = useMemo(() => topics.filter(t => `${t.title} ${t.original} ${t.conflict} ${t.markets} ${t.selection_group || ""} ${t.selected_by || ""}`.toLowerCase().includes(query.toLowerCase())), [query]);
  ...
  const headings: Record<string, [string, string]> = { projects: ["让每一个故事，走向交付。", "选题、分集、画布、审核都属于同一个项目。"], topics: ["好故事，先有明确的选择。", `原有 100 条选题快照 · 新增 ${teamSelectedTopics.length} 条团队已选题；先补齐方向，再关联立项。`], scripts: ["把创意，变成可审核的剧本。", "从项目打开批量剧本；多模型逐集对照，确认版本会回到项目审核和画布。"], ops: ["看见进度，也看见投入。", "查看模型、技能包、剧本运行记录与计划预算。" ] };
