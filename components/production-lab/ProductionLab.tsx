...
import { defaultProductionGroup, formatProjectScope, formatVisualStyle, PROJECT_CHANNEL_OPTIONS, PROJECT_LANGUAGE_OPTIONS, PROJECT_MARKET_OPTIONS, PROJECT_VISUAL_FORM_OPTIONS, selectionValues } from "../../lib/production-lab/project-form";
...
type ProductionGroup = { id: string; name: string };
...
function Desk({ demo = false, actor = demoActor, appearance = "light", onBackToWorkspace, onOpenWorkspace }: { demo?: boolean; actor?: Actor; appearance?: "light" | "dark"; onBackToWorkspace?: () => void; onOpenWorkspace?: (projectId: string) => void }) {
  ...
  async function refresh() {
    try {
      ...
      if (!response.ok) throw new Error(body.error); setState(body.state); setProductionGroups(Array.isArray(body.groups) ? body.groups : []); setError(""); setReady(true);
    } catch (e) { setError(e instanceof Error ? e.message : "加载失败"); }
  }
  useEffect(() => {
    if (!demo) { void refresh(); return; }
    try { const saved = localStorage.getItem(PRODUCTION_LAB_DEMO_STATE_KEY); const parsed = saved ? JSON.parse(saved) : demoState(); if (!Array.isArray(parsed.projects) || !Array.isArray(parsed.tasks)) throw new Error(); if (!saved) localStorage.setItem(PRODUCTION_LAB_DEMO_STATE_KEY, JSON.stringify(parsed)); setState(parsed); setProductionGroups([{ id: "demo-team", name: "试制一组" }]); setReady(true); }
  ...
  function openCreate(topic?: Topic) {
    ...
    const topicMarkets = topic ? selectionValues(topic.markets) : ["待确认"];
    const activeGroupNames = productionGroups.map(group => group.name);
    createForm.resetFields(); createForm.setFieldsValue({ topicId: topic?.id ?? null, title: topic?.title || "", source, brief: topic?.selection_group ? "团队已选题；故事梗概、改编来源与市场方向待补充。" : topic?.conflict || topic?.plot || "", tier: topic?.tier?.startsWith("★★★") ? "A" : "B", team: defaultProductionGroup(topic?.selection_group, activeGroupNames), markets: topicMarkets, languages: [], channels: [], visualForm: topic?.form ? [topic.form] : ["待确认"], styleNotes: topic?.style || "待确认", initialMinutes: 20, totalMinutes: 100, budgetCny: 0 }); setCreateOpen(true);
  }
  ...
  const nav = [{ key: "projects", label: "项目与进度", icon: LayoutDashboard }, { key: "topics", label: "选题提案", icon: BookOpen }, { key: "scripts", label: "剧本审核", icon: ListChecks }, { key: "ops", label: "用量与管理", icon: Settings2 }];
  const headings: Record<string, [string, string]> = { projects: ["让每一个故事，走向交付。", `团队已选 ${teamSelectedTopics.length} 条，分组与选择人已关联；从选题直接立项，制作全程归入同一项目。`], topics: ["好故事，先有明确的选择。", `团队已选 ${teamSelectedTopics.length} 条已置顶；下方继续浏览原有 100 条选题快照。`], scripts: ["审核已经写出的分集剧本。", "这里查看项目版本并做人工作业审核；要新建批次、对比 GPT / Claude 等文本模型，请回到制作画布打开“批量剧本”。"], ops: ["看见进度，也看见投入。", "查看模型、技能包、剧本运行记录与计划预算。" ] };
  const projectColumns = [
    { title: "制作项目", dataIndex: "title", render: (_: unknown, p: Project) => <button className={s.projectLink} onClick={() => setProjectId(p.id)}><span>{p.title}</span><small>制作小组：{p.team || "未分组"}</small><small>{p.topicId ? `选题 #${p.topicId} · ${p.topicSnapshot?.title || "已关联选题"} · ` : ""}{p.market} · {p.style}</small></button> },
  ...
  return <div className={`${s.root} ${appearance === "dark" ? s.dark : s.light}`}>
    ...
    <main className={s.main}><header className={s.topbar}><div>第六板块 <ChevronRight size={13} /> <strong>{nav.find(n => n.key === view)?.label}</strong></div><div className={s.topbarRight}><span className={s.pilot}><i />{demo ? "本机交互试用" : "内测白名单"}</span><Button type="primary" className={s.canvasReturn} icon={<ArrowLeft size={15} />} disabled={!onBackToWorkspace} onClick={onBackToWorkspace}>回到制作画布</Button></div></header>
      <div className={s.notice}>{demo ? "交互试用 · 含示例项目 · 数据仅保存在本浏览器 · 不调用模型、不产生费用" : "管理试用 · 使用独立数据库 · 批量剧本可多模型生成；图片 / 视频从制作画布的 WeToken 队列提交 · 全自动成片编排待接入"}</div>
