...
type Topic = { id: number; title: string; original: string; plot: string; markets: string[] | string; tier: string; form?: string; style: string; confidence: string; blocked?: boolean; source_rights: string; conflict: string; source_name: string };
...
const topics = topicData as unknown as Topic[];
...
function Desk({ demo = false, actor = demoActor, appearance = "light", onBackToWorkspace, onOpenWorkspace }: { demo?: boolean; actor?: Actor; appearance?: "light" | "dark"; onBackToWorkspace?: () => void; onOpenWorkspace?: (projectId: string) => void }) {
  ...
  function openCreate(topic?: Topic) {
    createForm.resetFields(); createForm.setFieldsValue({ topicId: topic?.id ?? null, title: topic?.title || "", source: topic ? `${topic.original}｜${topic.source_name || "推进台"} #${topic.id}；权利：${topic.source_rights || "待审核"}` : "原创提案", brief: topic?.conflict || topic?.plot || "", tier: topic?.tier?.startsWith("★★★") ? "A" : "B", market: topic ? String(topic.markets) : "待确认", style: topic ? `${topic.form || "形式待确认"} / ${topic.style}` : "待确认", team: "试制一组", initialMinutes: 20, totalMinutes: 100, budgetCny: 0 }); setCreateOpen(true);
  }
  const filteredTopics = useMemo(() => topics.filter(t => `${t.title} ${t.original} ${t.conflict} ${t.markets}`.toLowerCase().includes(query.toLowerCase())), [query]);
  ...
  const headings: Record<string, [string, string]> = { projects: ["让每一个故事，走向交付。", "选题、分集、画布、审核都属于同一个项目。"], topics: ["好故事，先有明确的选择。", "推进台 100 条选题快照 · 推荐保留为开发假设，立项时重新确认。"], scripts: ["把创意，变成可审核的剧本。", "从项目打开批量剧本；多模型逐集对照，确认版本会回到项目审核和画布。"], ops: ["看见进度，也看见投入。", "查看模型、技能包、剧本运行记录与计划预算。" ] };
  ...
  return <div className={`${s.root} ${appearance === "dark" ? s.dark : s.light}`}>
    ...
    <main className={s.main}><header className={s.topbar}><div>第六板块 <ChevronRight size={13} /> <strong>{nav.find(n => n.key === view)?.label}</strong></div><div className={s.topbarRight}><span className={s.pilot}><i />{demo ? "本机交互试用" : "内测白名单"}</span><Button type="primary" className={s.canvasReturn} icon={<ArrowLeft size={15} />} disabled={!onBackToWorkspace} onClick={onBackToWorkspace}>回到制作画布</Button></div></header>
      ...
      <div className={s.content}><div className={s.heading}><div><div className={s.eyebrow}>FG PRODUCTION / {view.toUpperCase()}</div><h1>{headings[view][0]}</h1><p>{headings[view][1]}</p></div><Button type="primary" size="large" icon={<Plus size={17} />} disabled={!ready} onClick={() => openCreate()}>新建立项</Button></div>
        ...
        {view === "topics" && <><div className={s.toolbar}><Input prefix={<Search size={16} />} placeholder="搜索故事、冲突或市场" value={query} onChange={e => { setQuery(e.target.value); setTopicPage(1); }} allowClear /><span>{filteredTopics.length} 条选题</span></div><div className={s.topicGrid}>{filteredTopics.slice((topicPage - 1) * 12, topicPage * 12).map(t => <article className={s.topicCard} key={t.id}><div className={s.topicTop}><span>#{String(t.id).padStart(3, "0")}</span><Tag>{t.confidence || "开发推断"}</Tag></div><h3>{t.title}</h3><small>{t.original}</small><p>{t.conflict || t.plot}</p><div className={s.topicMeta}>{String(t.markets)}<br />来源权利：{t.source_rights || "待审核"}</div><Button block disabled={!!t.blocked || !ready} onClick={() => openCreate(t)}>{t.blocked ? "暂缓开发" : "以此建立提案"}<ArrowUpRight size={14} /></Button></article>)}</div><div className={s.batchBar}><Pagination current={topicPage} pageSize={12} total={filteredTopics.length} onChange={setTopicPage} showSizeChanger={false} /></div><p className={s.muted}>推荐与市场标签来自推进台快照，未代表已验证的市场表现。</p></>}
        ...
        {view === "ops" && <>
          <div className={s.opsGrid}>
            <section className={s.panel}><div className={s.sectionTitle}><h2>预算边界</h2></div><div className={s.opsBody}><strong>¥ {state.projects.reduce((n, p) => n + p.budgetCny, 0).toLocaleString()}</strong><p>立项计划预算合计</p><Alert message="实际模型消耗：尚未接入账单对账" description="下方只显示真实文本模型请求与 token 回报；视频生成、供应商账单和人民币费用尚未接入。" type="info" /></div></section>
