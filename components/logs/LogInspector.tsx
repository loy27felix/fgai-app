...
export default function LogInspector({ row, state, onClose, onRetry }: { row: LogRecord | null; state: { status: 'idle' | 'loading' | 'ready' | 'error'; value: LogDetail | null; error: string }; onClose: () => void; onRetry: () => void }) {
  ...
  return (
    <aside className="log-desk__inspector" aria-label="日志详细信息">
      <div className="log-desk__inspector-head"><div><span className="log-desk__eyebrow">LOG DETAIL</span><h2>{row.event || '日志详情'}</h2></div><button type="button" onClick={onClose} aria-label="关闭详情">×</button></div>
