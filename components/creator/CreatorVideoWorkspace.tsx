...
import { confirmVideoTask, createVideoDraft, deleteVideoTask, finalizeVideoUploads, getVideoTask, listVideoTasks, reconcileVideoTask, CreatorImageClientError } from "@/lib/creator/video-client";
...
export default function CreatorVideoWorkspace({ userEmail }: Props) {
  ...
  const [knownWetokenTaskId, setKnownWetokenTaskId] = useState("");
  ...
  async function reconcileSelectedTask() {
    if (!selectedTask || reconciling) return;
    const externalTaskId = knownWetokenTaskId.trim();
    if (!/^cgt-[a-z0-9-]+$/i.test(externalTaskId)) {
      setReconciliationError("请输入完整的 Wetoken 任务号，格式例如 cgt-20260824150224-jjbsw。");
      return;
    }
    setReconciling(true); setReconciliationError("");
    try {
      const result = await reconcileVideoTask(selectedTask.id, externalTaskId);
  ...
  const historyView = tasks.map((task) => <div className={"video-history-row" + (task.id === selectedTaskId ? " active" : "")} key={task.id}><button type="button" onClick={() => { setSelectedTaskId(task.id); setConfirmTarget(task.status === "draft" && record(task.request).uploads_complete === true ? task : null); setKnownWetokenTaskId(task.external_task_id || ""); setReconciliationError(""); }}><span>{promptOf(task) || "未命名生视频任务"}</span><small>{statusLabel(task.status)} · {dateLabel(task.created_at)}</small></button>{["draft", "succeeded", "failed", "expired"].includes(task.status) && <button type="button" className="delete-small" onClick={() => setDeleteTarget(task)}><Icon d={I.trash} size={14} /></button>}</div>);
  ...
  function center() {
    ...
    if (selectedTask && REFRESHABLE_STATUSES.has(selectedTask.status)) {
      ...
      if (awaiting) return <div className="state card reconcile-card"><div className="mark"><Icon d={I.refresh} size={22} /></div><h1>提交状态未知，已停止自动等待</h1><p>{notice || selectedTask.error || "提交链路在等待供应商响应时中断。请先在 Wetoken 后台核对；若有 cgt 任务号，可只查询并归档结果，不会重新生成或再次扣费。"}</p><div className="reconcile-local-id"><span>本地任务</span><code>{selectedTask.id}</code></div><label>Wetoken 任务号<input value={knownWetokenTaskId} onChange={(event) => { setKnownWetokenTaskId(event.target.value); setReconciliationError(""); }} placeholder="cgt-20260824150224-jjbsw" autoComplete="off" spellCheck={false} /></label>{reconciliationError && <p className="reconcile-error">{reconciliationError}</p>}<div className="reconcile-actions"><button type="button" onClick={() => void pollTask()} disabled={reconciling}><Icon d={I.refresh} size={14} />刷新状态</button><button type="button" className="primary" onClick={() => void reconcileSelectedTask()} disabled={reconciling}>{reconciling ? "同步中…" : "查询并归档"}</button></div></div>;
      return <div className="state card"><div className="mark"><Icon d={I.refresh} size={22} /></div><h1>视频正在生成</h1><p>{notice || "状态会有限退避刷新，不会重复确认。"}</p><button type="button" onClick={() => void pollTask()}><Icon d={I.refresh} size={14} />刷新状态</button></div>;
    }
...
`}</style><style jsx>{`
.reconcile-card{text-align:left;max-width:570px}.reconcile-card .mark{margin:0}.reconcile-card label{display:block;margin-top:16px;color:var(--text-2);font-size:11px}.reconcile-card input{display:block;width:100%;height:40px;margin-top:7px;padding:0 10px;border:1px solid var(--stroke-2);border-radius:9px;background:var(--panel-solid);color:var(--text);font:12px "JetBrains Mono",monospace}.reconcile-local-id{display:flex;align-items:center;gap:9px;margin-top:16px;padding:9px 10px;border:1px solid var(--stroke);border-radius:9px;color:var(--text-3);font-size:10px}.reconcile-local-id code{overflow:hidden;color:var(--text-2);font-size:10px;text-overflow:ellipsis;white-space:nowrap}.reconcile-error{margin-top:9px!important;color:#ff9b85!important;font-size:11px!important}.reconcile-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}.reconcile-actions button{margin-top:0}.reconcile-actions .primary{border:0;background:var(--accent);color:var(--accent-ink);font-weight:700}.reconcile-actions button:disabled{opacity:.55;cursor:not-allowed}
`}</style></div>;
}
