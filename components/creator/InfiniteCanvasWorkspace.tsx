...
function initialSnapshot(kind: "image" | "video"): Snapshot {
  ...
  return { nodes: [text, output, config], connections: [{ id: nanoid(), fromNodeId: text.id, toNodeId: output.id }, { id: nanoid(), fromNodeId: config.id, toNodeId: output.id }], viewport: { x: 60, y: 40, k: .9 }, backgroundMode: "dots", title: kind === "video" ? "视频无限画布" : "图片无限画布" };
}
...
export default function InfiniteCanvasWorkspace({ userEmail, initialKind = "image" }: Props) {
  ...
  const output = selected && (selected.type === CanvasNodeType.Image || selected.type === CanvasNodeType.Video) ? selected : nodes.find((node) => node.type === (kind === "image" ? CanvasNodeType.Image : CanvasNodeType.Video)) || null;
  ...
  useEffect(() => { if (loaded.current) window.localStorage.setItem(STORE_KEY, JSON.stringify({ ...snapshot, viewport, backgroundMode })); }, [backgroundMode, snapshot, viewport]);
  ...
  const resetCanvas = useCallback(() => { const value = initialSnapshot(kind); setSnapshot(value); setViewport(value.viewport); setBackgroundMode(value.backgroundMode); setSelectedIds([]); setNotice("已新开画布，旧快照仍保存在本机。"); }, [kind]);
  const outputOptions = nodes.filter((node) => node.type === (kind === "image" ? CanvasNodeType.Image : CanvasNodeType.Video));

  return (
    <div className="fg-infinite-canvas" data-theme={theme}>
      <header className="fg-infinite-header"><div className="fg-infinite-brand"><span className="fg-infinite-logo">FG</span><span><strong>AI 创作台</strong><small>INFINITE CANVAS · PRIVATE</small></span></div><div className="fg-infinite-title"><span>画布工作台</span><strong>{snapshot.title}</strong></div><div className="fg-infinite-actions"><button type="button" className={kind === "image" ? "active" : ""} onClick={() => setKind("image")}><ImageIcon size={15} />图片</button><button type="button" className={kind === "video" ? "active" : ""} onClick={() => setKind("video")}><Video size={15} />视频</button><button type="button" onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>{theme === "dark" ? <Sun size={15} /> : <Moon size={15} />}</button><span className="fg-infinite-user">{userEmail.slice(0, 2).toUpperCase() || "FG"}</span></div></header>
      <div className="fg-infinite-body">
        <aside className="fg-infinite-left"><a href="/creator" className="fg-infinite-back">← 返回对话</a><button type="button" className="fg-infinite-new" onClick={resetCanvas}><Plus size={15} />新开画布</button><div className="fg-infinite-left-label">画布工具</div><button type="button" onClick={() => addNode(CanvasNodeType.Text, { x: 120, y: 120 })}><Plus size={14} />文本节点</button><button type="button" onClick={() => addNode(CanvasNodeType.Image, { x: 180, y: 280 })}><ImageIcon size={14} />图片节点</button><button type="button" onClick={() => addNode(CanvasNodeType.Video, { x: 560, y: 280 })}><Video size={14} />视频节点</button><button type="button" onClick={() => addNode(CanvasNodeType.Config, { x: 120, y: 480 })}><Database size={14} />生成配置</button><div className="fg-infinite-hint">右键画布添加节点<br />拖动节点端口建立参考关系<br />选中输出节点后在右侧生成</div><div className="fg-infinite-left-footer">草稿 → 明确确认 → 模型调用。<br />浏览器不会保存 Wetoken key。</div></aside>
