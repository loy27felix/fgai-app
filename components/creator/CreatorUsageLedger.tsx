...
const emptyData: UsageResponse = {
  records: [],
  count: 0,
  totals: { calls: 0, inputTokens: 0, outputTokens: 0, totalTokens: 0, images: 0, videoSeconds: 0, knownCostUsd: 0, unpriced: 0 },
};
...
function costLabel(record: UsageRecord) {
  const cost = record.reported_cost_usd ?? record.estimated_cost_usd;
  if (cost === null) return '待核算';
  return `${record.currency === 'USD' ? '$' : `${record.currency} `}${cost.toFixed(6)}`;
}
...
export default function CreatorUsageLedger() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setData({ records: Array.isArray(payload.records) ? payload.records : [], count: Number(payload.count || 0), totals: { ...emptyData.totals, ...(payload.totals || {}) } });
      setError('');
    } catch (cause) {
  ...
  const summaryLabel = useMemo(() => {
    ...
    return data.totals.knownCostUsd > 0 ? `已核算 $${data.totals.knownCostUsd.toFixed(4)}` : `${data.totals.calls} 次调用`;
  }, [data.totals.calls, data.totals.knownCostUsd]);
        ...
        <div role="presentation" onMouseDown={() => setOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'grid', placeItems: 'center', padding: 20, background: 'rgba(3,7,14,.62)', backdropFilter: 'blur(8px)' }}>
          <section role="dialog" aria-modal="true" aria-labelledby="fg-usage-title" onMouseDown={(event) => event.stopPropagation()} style={{ width: 'min(720px, 100%)', maxHeight: 'min(760px, 90vh)', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '1px solid var(--stroke-2, rgba(120,130,150,.35))', borderRadius: 18, background: 'var(--panel-solid, #fff)', color: 'var(--text, #111)', boxShadow: '0 30px 100px rgba(0,0,0,.35)' }}>
            <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '16px 18px', borderBottom: '1px solid var(--stroke, rgba(120,130,150,.2))' }}>
              <div><div id="fg-usage-title" style={{ fontSize: 15, fontWeight: 700 }}>生成与费用记录</div><div style={{ marginTop: 4, color: 'var(--text-3, #777)', fontSize: 11 }}>只显示当前账号；生成开始前会先写入记录，金额以供应商账单为准。</div></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8, padding: 14, borderBottom: '1px solid var(--stroke, rgba(120,130,150,.2))' }}>
              ...
              <Summary label="已核算" value={data.totals.knownCostUsd ? `$${data.totals.knownCostUsd.toFixed(6)}` : '待核算'} />
            </div>
            ...
            <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 14 }}>
              ...
              {data.records.map((record) => <div key={record.id || record.request_id} style={{ display: 'grid', gridTemplateColumns: '58px minmax(0, 1fr) auto', gap: 10, alignItems: 'center', padding: '11px 4px', borderBottom: '1px solid var(--stroke, rgba(120,130,150,.16))' }}>
                ...
                <div style={{ textAlign: 'right' }}><div style={{ color: statusColor(record.status), fontSize: 11 }}>{statusLabel(record.status)}</div><div style={{ marginTop: 3, color: record.cost_source === 'unknown' ? '#e6b85c' : 'var(--text-2, #555)', fontSize: 10 }}>{costLabel(record)}</div></div>
              </div>)}
            </div>
            <footer style={{ padding: '10px 16px', borderTop: '1px solid var(--stroke, rgba(120,130,150,.2))', color: 'var(--text-3, #777)', fontSize: 10 }}>最近显示 {data.records.length} / {data.count} 条；{data.totals.unpriced ? `${data.totals.unpriced} 条等待供应商价格核算。` : '价格均已返回。'}</footer>
          </section>
        </div>
      ) : null}
    </>
