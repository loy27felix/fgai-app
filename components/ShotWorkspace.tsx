                    ...
                    <div style={{ flex: "none", borderRadius: 18, background: "var(--panel)", border: "1px solid var(--stroke)", boxShadow: "var(--inset)", overflow: "hidden" }}>
                      ...
                      <div style={{ padding: "16px 18px", display: "flex", gap: 16, flexWrap: "wrap" }}>
                        <div style={{ flex: 1, minWidth: 280 }}>
                          <div style={{ aspectRatio: "16/9", borderRadius: 12, overflow: "hidden", background: "var(--bg-2)", border: "1px solid var(--stroke-2)", display: "grid", placeItems: "center", color: "var(--text-3)", fontSize: 12, position: "relative" }}>{fu(sel.keyframe_path) ? <img src={fu(sel.keyframe_path) as string} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : (busy === "keyframe_path" + sel.id ? "生成中…" : "尚无关键帧")}<a href={`/projects/${projectId}/shots/canvas`} title="画布编辑" style={{ position: "absolute", right: 8, top: 8, display: "flex", alignItems: "center", gap: 5, fontSize: 9.5, color: "#fff", padding: "3px 8px", borderRadius: 7, background: "rgba(0,0,0,.42)" }} className="fg-mono"><Icon d={["M6 6m-2.5 0a2.5 2.5 0 1 0 5 0a2.5 2.5 0 1 0 -5 0", "M18 12m-2.5 0a2.5 2.5 0 1 0 5 0a2.5 2.5 0 1 0 -5 0", "M8.2 7 15.5 11"]} size={11} sw={1.8} />画布</a></div>
                        </div>
