...
import { openWorkerInput, WorkerStorageError, workerLeaseTokenFromRequest } from "@/lib/local/worker-storage";
import { localFileSize, readLocalRange } from "@/lib/local/storage";

export const runtime = "nodejs";

function errorResponse(error: unknown) {
  const typed = error as { message?: string; code?: string; status?: number };
  return NextResponse.json({ error: typed.message || "读取任务输入失败", code: typed.code || "WORKER_INPUT_FAILED" }, { status: typed.status || 500 });
}
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const input = await openWorkerInput(worker, params.id, leaseToken);
    const range = parseRange(request.headers.get("range"), input.size);
    const body = await readLocalRange(input.bucket, input.storagePath, range.start, range.end);
    return new Response(body, {
      ...
      headers: {
        ...
        "accept-ranges": "bytes",
        ...(range.partial ? { "content-range": `bytes ${range.start}-${range.end}/${input.size}` } : {}),
