...
export default async function VideoPage({ params }: { params: { id: string } }) {
  ...
  const { data: shots } = sceneIds.length ? await supabase.from("shots").select("id, scene_id, no, title, duration_s, keyframe_path, frame_path, video_prompt, video_url, roles").in("scene_id", sceneIds).order("no") : { data: [] as any[] };
