...
export default async function BgmPage({ params }: { params: { id: string } }) {
  ...
  const { data: scenes } = episodeIds.length
    ? await localClient.from('scenes').select('id,episode_id,idx,title,setting').in('episode_id', episodeIds).order('idx')
    : { data: [] as any[] };
  const sceneIds = (scenes || []).map((scene: any) => scene.id);
  const { data: shots } = sceneIds.length
    ? await localClient.from('shots').select('id,scene_id,no,title,duration_s,script_beat,video_prompt').in('scene_id', sceneIds).order('no')
    : { data: [] as any[] };
