...
export default async function ShotsPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: shots }, { data: scripts }] = sceneIds.length
    ? await Promise.all([
        localClient.from("shots").select("id, scene_id, no, title, time_start, time_end, duration_s, script_beat, roles, frame_path, keyframe_path, storyboard_path, keyframe_prompt, storyboard_prompt, video_prompt, outputs").in("scene_id", sceneIds).order("no"),
        localClient.from("scripts").select("scene_id, body").in("scene_id", sceneIds),
      ])
    : [{ data: [] as any[] }, { data: [] as any[] }];
  const scriptText = allScenes.map((s) => { const ep = (episodes || []).find((e: any) => e.id === s.episode_id); const body = (scripts || []).find((x: any) => x.scene_id === s.id)?.body || ""; return body ? `【第${ep?.idx || "?"}集·第${s.idx}场】\n${body}` : ""; }).filter(Boolean).join("\n\n").slice(0, 8000);
