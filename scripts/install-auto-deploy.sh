...
LABEL="com.fgstudio.auto-deploy"
...
[[ "$(git -C "$PROJECT_ROOT" branch --show-current)" == "main" ]] || {
  printf 'The deployment checkout must be on main.\n' >&2
  exit 1
}

mkdir -p "$HOME/Library/LaunchAgents" "$LOG_DIR"

TEMP_PLIST="$(mktemp "${TMPDIR:-/tmp}/fg-studio-auto-deploy.XXXXXX")"
cleanup() {
  rm -f "$TEMP_PLIST"
}
trap cleanup EXIT
...
<dict>
  ...
  <array>
    <string>$PROJECT_ROOT/scripts/auto-deploy.sh</string>
  </array>
  ...
  <string>$LOG_DIR/fg-studio-auto-deploy.log</string>
  <key>StandardErrorPath</key>
  <string>$LOG_DIR/fg-studio-auto-deploy.error.log</string>
</dict>
</plist>
PLIST
...
printf 'Auto deploy installed: %s\n' "$PLIST_PATH"
