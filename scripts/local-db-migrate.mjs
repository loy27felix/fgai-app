...
const migrationName = "002-local-upgrade.sql";
...
async function main() {
  ...
  try {
    ...
    log("connected", { migration: migrationName });
    ...
    console.error(`${prefix} ${JSON.stringify({ event: "local_db_migration", stage: "failed", migration: migrationName, ...errorDetails(error) })}`);
    process.exitCode = 1;
  } finally {
